...
export class ScannerService {
  private isScanning: boolean = false; // Bandera para controlar el estado del escáner
  ...
  async scan(): Promise<string> {
    // Si ya se está escaneando, se evita iniciar un nuevo escaneo
    if (this.isScanning) {
      console.log('El escáner ya está en uso');
      return Promise.resolve('');
    }

    // Se marca como escaneando
    this.isScanning = true;
    
    try {
      const result = await this.runGoogleBarcodeScanner();
      return result;
    } finally {
      // Finalmente, se marca como no escaneando, independientemente de si hubo error o no
